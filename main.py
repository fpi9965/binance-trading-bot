"""
بوت التداول الآلي - صفقات متعددة
"""
import time
import threading
import os
...
def run_bot():
    ...
    try:
        ...
        notifier.send_message("🟢 *تم تشغيل البوت - صفقات متعددة!*\n\n"
                ...
                if open_count > 0:
                    ...
                    if results:
                        ...
                        for symbol, data in sorted(results.items(), key=lambda x: x[1]['score'], reverse=True):
                            if data['recommendation'] == 'BUY' and data['score'] >= 20:
                                notifier.send_trade_signal(symbol, data)
                        
                        # فتح صفقات متعددة
                        top_picks = analysis.get_top_picks(results, top_n=config.MAX_POSITIONS)
                        ...
                        if top_picks and config.TEST_MODE == False:
                            ...
                            for idx, (symbol, data) in enumerate(top_picks):
                                ...
                                print(f"\n🏆 الصفقة {idx+1}: {symbol} (درجة: {data['score']})")
                                sys.stdout.flush()
