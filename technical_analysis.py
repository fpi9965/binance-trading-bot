...
class TechnicalAnalysis:
    ...
    def analyze(self, symbol):
        try:
            klines = self.binance.get_klines(symbol, '15m', 100)
            if not klines:
                return None
            ...
            if macd['macd'] > macd['signal']:
                ...
                score -= 10
            if score >= 60:
                return {'action': 'buy', 'score': score, 'rsi': rsi, 'current_price': closes[-1]}
            elif score <= 20:
                return {'action': 'sell', 'score': score, 'rsi': rsi, 'current_price': closes[-1]}
